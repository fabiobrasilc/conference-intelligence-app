    ...
    if response_type == 'followup':
        ...
        if user_query.strip().lower() in ['yes', 'yes please', 'yeah', 'yep', 'sure', 'ok', 'okay']:
            ...
            if 'broaden' in context_query.lower() or 'keywords' in context_query.lower():
                ...
                # Match everything after "keywords:" until we hit "or", ")", or end of string
                keyword_match = re.search(r'keywords?:\s*([^)]+?)(?:\s+or\b|$)', context_query, re.IGNORECASE)
                if keyword_match:
                    keywords_text = keyword_match.group(1).strip()
                    # Split by commas and clean up
                    search_terms = [k.strip(' "\',') for k in keywords_text.split(',') if k.strip()]
