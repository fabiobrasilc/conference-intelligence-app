...
from pathlib import Path
CSV_FILE = Path(__file__).parent / "esmo2025_all.csv"
...
def get_filtered_dataframe(drug_filter: str = "All EMD Portfolio", ta_filter: str = "All Therapeutic Areas") -> pd.DataFrame:
    ...
    # Stage 2: Therapeutic Area Filtering - DISABLED for now (focus on drug search only)
    # TODO: Re-enable when TA filtering logic is clarified
    # if ta_filter != "All Therapeutic Areas" and ta_filter in ESMO_THERAPEUTIC_AREAS:
    #     ta_filters = ESMO_THERAPEUTIC_AREAS[ta_filter]
    #     if ta_filters:
    #         ta_mask = df["main_filters"].isin(ta_filters)
    #         df = df[ta_mask]
...
def load_and_prepare_esmo_data():
    ...
    # ESMO schema: main_filters,session_category,date,time,room,session_type,identifier,study_title,speaker,location,affiliation
    print(f"Loaded ESMO data: {len(df)} sessions")
    print(f"Columns: {list(df.columns)}")

    # Keep original columns for display AND create renamed columns for compatibility
    # First preserve the original columns
    original_cols = df.copy()

    # Create renamed columns for existing functionality
    ...
    df["ta"] = df["main_filters"]  # Map therapeutic area column for ESMO compatibility
...
def get_data_api():
    ...
    # Use original CSV columns exactly as they appear in the ESMO data
    display_columns = ["main_filters", "session_category", "date", "time", "room", "session_type", "identifier", "study_title", "speaker", "location", "affiliation"]
...
def search_data_api():
    ...
    # Search only in relevant text fields (not main_filters which contains therapeutic areas)
    search_columns = ['study_title', 'speaker', 'affiliation', 'session_category']
    ...
    # Use ESMO column structure for search results display
    display_columns = ["main_filters", "session_category", "date", "time", "room", "session_type", "identifier", "study_title", "speaker", "location", "affiliation"]
