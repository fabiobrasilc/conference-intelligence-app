...
def sanitize_unicode_for_windows(text):
    """
    Replace Unicode characters that can't be encoded with cp1252 codec on Windows.
...
def sanitize_data_structure(data):
    """
    Recursively sanitize Unicode characters in any data structure (dict, list, string).
    Used for cleaning table data before JSON serialization.
    """
...
def apply_bladder_cancer_filter(df: pd.DataFrame) -> pd.Series:
    """Apply bladder cancer filter with exclusion for prostate cancer"""
    keywords = ["bladder", "urothelial", "uroepithelial", "transitional cell", "GU", "genitourinary"]
    exclusions = ["prostate"]
    ...
    # Exclude prostate cancer results
...
def apply_lung_cancer_filter(df: pd.DataFrame) -> pd.Series:
    """Apply lung cancer filter with NSCLC focus"""
    keywords = ["NSCLC", "non-small cell lung cancer", "non-small-cell lung cancer", "MET", "ALK", "EGFR", "KRAS"]
    mask = pd.Series([False] * len(df))
    for keyword in keywords:
        title_mask = df["Title"].str.contains(keyword, case=False, na=False)
...
def get_filtered_dataframe_multi(drug_filters: List[str], ta_filters: List[str], session_filters: List[str] = None, date_filters: List[str] = None) -> pd.DataFrame:
    ...
    Uses AND logic: results must match ALL selected filter types.
    ...
    # Start with full dataset and apply each filter type as AND constraint
    result_df = df_global.copy()

    # Apply drug filters (OR within drug type, but AND with other filter types)
    if drug_filters:
        drug_masks = []
        for drug_filter in drug_filters:
            if drug_filter == "All EMD Portfolio":
                # All EMD Portfolio means no drug filtering
                drug_masks.append(pd.Series([True] * len(result_df)))
            elif drug_filter == "Avelumab Focus":
                drug_mask = result_df["Title"].str.contains("avelumab|bavencio", case=False, na=False)
                drug_masks.append(drug_mask)
            elif drug_filter == "Tepotinib Focus":
                drug_mask = result_df["Title"].str.contains("tepotinib", case=False, na=False)
                drug_masks.append(drug_mask)
            elif drug_filter == "Cetuximab Focus":
                drug_mask = result_df["Title"].str.contains("cetuximab", case=False, na=False)
                drug_masks.append(drug_mask)

        # Combine drug masks with OR logic (any selected drug)
        if drug_masks:
            combined_drug_mask = drug_masks[0]
            for mask in drug_masks[1:]:
                combined_drug_mask = combined_drug_mask | mask
            result_df = result_df[combined_drug_mask]

    # Apply TA filters (OR within TA type, but AND with other filter types)
    if ta_filters:
        ta_masks = []
        for ta_filter in ta_filters:
            if ta_filter == "All Therapeutic Areas":
                # All TAs means no TA filtering
                ta_masks.append(pd.Series([True] * len(result_df)))
            elif ta_filter == "Bladder Cancer":
                ta_mask = apply_bladder_cancer_filter(result_df)
                ta_masks.append(ta_mask)
            elif ta_filter == "Renal Cancer":
                ta_mask = apply_renal_cancer_filter(result_df)
                ta_masks.append(ta_mask)
            elif ta_filter == "Lung Cancer":
                ta_mask = apply_lung_cancer_filter(result_df)
                ta_masks.append(ta_mask)
            elif ta_filter == "Colorectal Cancer":
                ta_mask = apply_colorectal_cancer_filter(result_df)
                ta_masks.append(ta_mask)
            elif ta_filter == "Head & Neck Cancer":
                ta_mask = apply_head_neck_cancer_filter(result_df)
                ta_masks.append(ta_mask)
            elif ta_filter == "TGCT":
                ta_mask = apply_tgct_filter(result_df)
                ta_masks.append(ta_mask)

        # Combine TA masks with OR logic (any selected TA)
        if ta_masks:
            combined_ta_mask = ta_masks[0]
            for mask in ta_masks[1:]:
                combined_ta_mask = combined_ta_mask | mask
            result_df = result_df[combined_ta_mask]

    # Apply session filters (OR within session type, but AND with other filter types)
    if session_filters and session_filters != ["All Session Types"]:
        result_df = apply_session_filter(result_df, session_filters)

    # Apply date filters (OR within date type, but AND with other filter types)
    if date_filters and date_filters != ["All Days"]:
        result_df = apply_date_filter(result_df, date_filters)
    ...
    def generate():
        try:
            ...
            if plan.response_type == "specific_lookup":
                ...
                if institution_name:
                    # Handle institution lookup - use same multi-column search as Search API
                    # Search across all relevant columns like the Search API does
                    search_columns = ['Title', 'Speakers', 'Speaker Location', 'Affiliation', 'Identifier', 'Room', 'Date', 'Time', 'Session', 'Theme']
                    mask = pd.Series([False] * len(filtered_df))

                    # Search each column and combine with OR logic (like Search API)
                    for col in search_columns:
                        if col in filtered_df.columns:
                            col_mask = safe_contains(filtered_df[col], institution_name, regex=True)
                            mask = mask | col_mask

                    # Don't use drop_duplicates by Identifier - it removes valid results with NaN identifiers
                    institution_results = filtered_df.loc[mask, ["Identifier","Session","Title","Speakers","Affiliation"]]
...
Respond naturally to exactly what the user asked about this institution."""
                    ...
                    # Stream AI analysis of the institution data using the streaming_prompt
                    stream = client.responses.create(
                        model="gpt-5-mini",
                    ...
                    for event in stream:
                        if event.type == "response.output_text.delta":
                            token = event.delta
                            sanitized_token = sanitize_unicode_for_windows(token)
                            import json
                            yield f"data: {json.dumps({'text': sanitized_token})}\n\n"
...
Respond naturally to exactly what the user asked - don't follow rigid frameworks or bullet points unless they specifically request that format."""
                    ...
                    # Stream AI analysis of the author data using the streaming_prompt
                    stream = client.responses.create(
                        model="gpt-5-mini",
                        input=[{"role": "user", "content": streaming_prompt}],
                        reasoning={"effort": "low"},
                    ...
                    for char in simple_response:
                        import json
                        sanitized_char = sanitize_unicode_for_windows(char)
                        yield f"data: {json.dumps({'text': sanitized_char})}\n\n"
                    yield f"data: [DONE]\n\n"
                    return
                ...
                if drug_entities:
                    ...
                    if not drug_results.empty:
                        ...
                        # Sanitize table data for Windows Unicode compatibility
                        sanitized_table_data = {
                            "title": sanitize_unicode_for_windows(table_title),
                            "rows": sanitize_data_structure(table_data)
                        }

                        yield sse_event("table", sanitized_table_data)
...
Provide a brief summary of these findings, mentioning the count and any notable patterns in the research areas or institutions involved."""
                    ...
                    # Check for institution entities (like MD Anderson)
                    institution_entities = plan.primary_entities.get("institutions", [])

                    if institution_entities:
                        # Search for institution mentions in the dataset
                        institution_name = institution_entities[0]

                        # Use same multi-column search as Search API (not just Affiliation)
                        search_columns = ['Title', 'Speakers', 'Speaker Location', 'Affiliation', 'Identifier', 'Room', 'Date', 'Time', 'Session', 'Theme']
                        mask = pd.Series([False] * len(filtered_df))
                        ...
                        for col in search_columns:
                            if col in filtered_df.columns:
                                col_mask = filtered_df[col].astype(str).str.contains(institution_name, case=False, na=False)
                                mask = mask | col_mask

                        institution_results = filtered_df[mask]

                        if not institution_results.empty:
                            # Generate table with institution studies
                            table_data = institution_results[["Identifier","Session","Title","Speakers","Affiliation"]].to_dict('records')
                            table_title = f"{institution_name} Studies - ESMO 2025 ({len(institution_results)} sessions)"
                            ...
                            # Stream response about the findings
                            institution_data = f"Found {len(institution_results)} studies from MD Anderson:\n\n" + institution_results[["Identifier","Title","Speakers"]].to_csv(index=False)

                            streaming_prompt = f"""Based on the ESMO 2025 conference data, I found {len(institution_results)} studies from MD Anderson Cancer Center.

Study Details:
{institution_data}

Provide a brief summary of these findings, mentioning the count and any notable patterns in the research areas or therapeutic focus represented by MD Anderson at this conference."""

                            # Stream AI analysis of the institution data
                            stream = client.responses.create(
                                model="gpt-5-mini",
                                input=[{"role": "user", "content": streaming_prompt}],
                                reasoning={"effort": "low"},
                            ...
                            # No institution data found - stream simple response
                            simple_response = f"I searched the ESMO 2025 dataset but couldn't find any studies from {institution_name}. This could be because the institution name appears differently in the dataset or they may not have presented at this conference."
                ...
                try:
                    for event in stream:
                        if event.type == "response.output_text.delta":
                            ...
                            sanitized_token = sanitize_unicode_for_windows(token)
                            accumulated_content += sanitized_token
