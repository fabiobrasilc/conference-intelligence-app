...
# Conference management globals
current_conference = None
conference_configs = None
...
def get_filtered_dataframe(ta_filter: str, conference_id: str = None) -> pd.DataFrame:
    """
    Unified function to filter dataframe by therapeutic area and conference.
    Uses the new conference configuration system for multi-conference support.

    Args:
        ta_filter: Therapeutic area filter ("Bladder Cancer", "Renal Cell Carcinoma", "All", etc.)
        conference_id: Conference identifier (defaults to current_conference)

    Returns:
        Filtered dataframe copy
    """
    global current_conference, conference_configs, df_global

    if df_global is None:
        return pd.DataFrame()

    # Use current conference if not specified
    if conference_id is None:
        conference_id = current_conference or "ASCO_GU_2025"

    # Get conference configuration
    if conference_configs is None:
        conference_configs = get_conference_configs()

    config = conference_configs.get(conference_id)
    if config is None:
        # Fallback to ASCO structure for backward compatibility
        if ta_filter == "Bladder Cancer":
            return df_global[df_global["ta"] == "bladder"].copy()
        elif ta_filter == "Renal Cell Carcinoma":
            return df_global[df_global["ta"] == "renal"].copy()
        else:
            return df_global.copy()

    # Apply conference-aware filtering
    if ta_filter == "All":
        return df_global.copy()

    # Use the conference's TA column and mapping
    ta_column = config.ta_column
    if ta_column not in df_global.columns:
        return df_global.copy()

    # Map the filter to the conference's internal values
    reverse_mapping = {v: k for k, v in config.ta_mapping.items()}
    internal_value = reverse_mapping.get(ta_filter)

    if internal_value:
        return df_global[df_global[ta_column] == internal_value].copy()
    else:
        # Try direct match if mapping doesn't exist
        return df_global[df_global[ta_column].str.contains(ta_filter, case=False, na=False)].copy()

def load_conference_data(conference_id: str) -> pd.DataFrame:
    """
    Load and normalize data for a specific conference
    """
    global conference_configs
    ...
    csv_path = Path(__file__).parent / config.csv_file
    try:
        df = pd.read_csv(csv_path, encoding=config.encoding)
    except UnicodeDecodeError:
        # Fallback encoding
        df = pd.read_csv(csv_path, encoding='latin-1')

    # Apply column mapping to create unified schema
    if config.column_mapping:
        # Create reverse mapping for renaming
        rename_mapping = {}
        for original_col, standard_col in config.column_mapping.items():
            if original_col in df.columns:
                rename_mapping[original_col] = standard_col

        df = df.rename(columns=rename_mapping)

    # Ensure standard columns exist
    standard_columns = ["abstract_id", "session_id", "title", "authors", "institutions", "therapeutic_area"]
...
def get_available_therapeutic_areas(conference_id: str = None) -> List[str]:
    """
    Get list of available therapeutic areas for a conference
    """
    global current_conference, conference_configs
    ...
    config = conference_configs.get(conference_id)
    if config is None:
        return ["All", "Bladder Cancer", "Renal Cell Carcinoma"]

    # Return the standardized TA names
    areas = ["All"] + list(config.ta_mapping.values())
    return list(dict.fromkeys(areas))  # Remove duplicates while preserving order
...
def initialize_app_globals(conference_id: str = None):
    """
    Initialize application globals with conference-aware data loading
    """
    global df_global, bladder_keywords_global, renal_keywords_global, csv_hash_global, collection
    global current_conference, conference_configs

    # Set default conference if not specified
    if conference_id is None:
        conference_id = current_conference or "ASCO_GU_2025"

    # Initialize conference configs if needed
    if conference_configs is None:
        conference_configs = get_conference_configs()

    # Check if we need to reload data for a different conference
    should_reload = (df_global is None or
                    current_conference != conference_id)

    if should_reload:
        print(f"Initializing application globals for {conference_id}...")
        try:
            # Update current conference
            current_conference = conference_id

            # Load conference-specific data
            if conference_id in conference_configs:
                # Use new unified data loading
                df_global = load_conference_data(conference_id)
                csv_hash_global = hashlib.md5(str(df_global.values.tobytes()).encode()).hexdigest()
            else:
                # Fallback to legacy loading for ASCO
                df_global, bladder_keywords_global, renal_keywords_global, csv_hash_global = load_and_prepare_data()

            print(f"Data loaded successfully for {conference_id}. Hash: {csv_hash_global[:8]}")
            ...
            print(f"FATAL ERROR: Data file not found for {conference_id}: {e}")
            return False
        except Exception as e:
            print(f"FATAL ERROR during {conference_id} initialization: {e}")
            return False

    return True

def switch_conference(new_conference_id: str) -> bool:
    """
    Switch to a different conference and reload data
    """
    global current_conference

    if new_conference_id == current_conference:
        return True  # Already on this conference

    # Force reload by clearing current data
    global df_global, collection, csv_hash_global
    df_global = None
    collection = None
    csv_hash_global = None

    # Initialize with new conference
    return initialize_app_globals(new_conference_id)
...
def get_conferences():
    ...
    try:
        ...
        conferences = []
        for conf_id, config in conference_configs.items():
            conferences.append({
                "id": conf_id,
                "name": config.name,
                "has_multi_authors": config.has_multi_authors,
                "has_geographic_data": config.has_geographic_data,
                "has_session_metadata": config.has_session_metadata,
                "affiliation_quality": config.affiliation_quality
            })

        return jsonify({
            "conferences": conferences,
            "current": current_conference or "ASCO_GU_2025"
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/conference/switch', methods=['POST'])
def switch_conference_api():
    """Switch to a different conference"""
    try:
        data = request.get_json()
        new_conference_id = data.get('conference_id')
        ...
        if new_conference_id not in conference_configs:
            return jsonify({"error": f"Unknown conference: {new_conference_id}"}), 400

        # Switch conference
        success = switch_conference(new_conference_id)
        if success:
            # Get available therapeutic areas for this conference
            ta_areas = get_available_therapeutic_areas(new_conference_id)
            return jsonify({
                "success": True,
                "conference_id": new_conference_id,
                "therapeutic_areas": ta_areas
            })
        else:
            return jsonify({"error": "Failed to switch conference"}), 500

    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/conference/current')
def get_current_conference():
    ...
    try:
        ...
        conf_id = current_conference or "ASCO_GU_2025"
        config = conference_configs.get(conf_id)

        if config:
            return jsonify({
                "id": conf_id,
                "name": config.name,
                "therapeutic_areas": get_available_therapeutic_areas(conf_id),
                "features": {
                    "multi_authors": config.has_multi_authors,
                    "geographic_data": config.has_geographic_data,
                    "session_metadata": config.has_session_metadata
                },
                "data_quality": {
                    "affiliation_quality": config.affiliation_quality,
                    "affiliation_source": config.affiliation_source
                }
            })
        else:
            return jsonify({"error": "Conference configuration not found"}), 404
